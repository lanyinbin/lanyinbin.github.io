// ============================================================
// 复兴号 G88 高铁体验 · 西安北→郑州东→北京西（回程）
// 8阶段状态机:
// MAP → STATION_DZ → BOARDING_WALK → CABIN_SEATING → DRIVING
...
(function () {
    ...
    const AUDIO_FALLBACK = {
        ...
        boarding: '旅客您好，G88次列车即将开车，请抓紧时间上车。',
        depart: '各位旅客，列车出发了！请坐好扶稳！',
        'speed-200': '现在时速200公里！',
        'speed-300': '哇，300公里每小时了！复兴号真快！',
        'speed-350': '350公里每小时！这就是复兴号的最高速度！',
        'arriving-langfang': '前方到站，郑州东。',
        'langfang-stop': '郑州东站到了。',
        'langfang-depart': '郑州东站开车，下一站，终点站北京西。',
        'arriving-beijing': '前方终点站，北京西。',
        arrived: '北京西站到了！全程1200公里，感谢您乘坐G88次复兴号列车！',
    };
    ...
    // 郑州东停站状态
    let langfangState = 'approaching'; // approaching | stopping | stopped | departing | passed
    const LANGFANG_APPROACH_KM = 20;
    const LANGFANG_STOP_DURATION = 4;
    let langfangStopTimer = 0;
    ...
    function phaseToStatusText() {
        if (phase === Phase.DRIVING) {
            ...
            if (langfangState === 'stopped') return '停靠郑州东';
            if (langfangState === 'stopping') return '进站减速';
            if (langfangState === 'departing') return '郑州东发车';
            if (langfangState === 'passed') return '驶向北京西';
            return '行驶中';
        }
        if (phase === Phase.CABIN_SEATING) return '16车 寻找10F';
    ...
    function addRouteOverlay(svgEl, bjEl, sxEl, forMiniMap) {
        ...
        // 路线：西安北 → 郑州东 → 北京西
        addSvgLine(svgEl, ns, dzCx, dzCy, lfCx, lfCy, 'route-glow');
        addSvgLine(svgEl, ns, dzCx, dzCy, lfCx, lfCy, 'route-line');
        addSvgLine(svgEl, ns, lfCx, lfCy, bjCx, bjCy, 'route-glow');
        addSvgLine(svgEl, ns, lfCx, lfCy, bjCx, bjCy, 'route-line');

        addCityMarker(svgEl, ns, dzCx, dzCy, '西安北', '', 16);
        addCityMarker(svgEl, ns, lfCx, lfCy, '郑州东', 'mid-label', -12);
        addCityMarker(svgEl, ns, bjCx, bjCy, '北京西', 'dest-label', -12);
        ...
        if (!forMiniMap) {
            ...
            mapProgressDot.setAttribute('stroke', '#DAA520');
            mapProgressDot.setAttribute('stroke-width', '1.8');
            mapProgressDot.setAttribute('cx', dzCx);
            mapProgressDot.setAttribute('cy', dzCy);
    ...
    function buildMiniMap(miniSvgEl, geo) {
        ...
        const pad = 34;
        const minX = Math.min(geo.dz.x, geo.lf.x, geo.bj.x) - pad;
        const minY = Math.min(geo.dz.y, geo.lf.y, geo.bj.y) - pad;
        const maxX = Math.max(geo.dz.x, geo.lf.x, geo.bj.x) + pad;
        const maxY = Math.max(geo.dz.y, geo.lf.y, geo.bj.y) + pad;
        miniSvgEl.setAttribute('viewBox', `${minX} ${minY} ${maxX - minX} ${maxY - minY}`);

        miniMapData.svg = miniSvgEl;
        miniMapData.bgLine1 = addSvgLine(miniSvgEl, ns, geo.dz.x, geo.dz.y, geo.lf.x, geo.lf.y);
        miniMapData.bgLine2 = addSvgLine(miniSvgEl, ns, geo.lf.x, geo.lf.y, geo.bj.x, geo.bj.y);
        miniMapData.doneLine1 = addSvgLine(miniSvgEl, ns, geo.dz.x, geo.dz.y, geo.dz.x, geo.dz.y);
        miniMapData.doneLine2 = addSvgLine(miniSvgEl, ns, geo.lf.x, geo.lf.y, geo.lf.x, geo.lf.y);
        ...
        miniMapData.stationDot1.setAttribute('cx', geo.dz.x);
        miniMapData.stationDot1.setAttribute('cy', geo.dz.y);
        ...
        miniMapData.stationDot2.setAttribute('cx', geo.lf.x);
        miniMapData.stationDot2.setAttribute('cy', geo.lf.y);
        ...
        miniMapData.stationDot3.setAttribute('cx', geo.bj.x);
        miniMapData.stationDot3.setAttribute('cy', geo.bj.y);
        ...
        miniMapData.trainDot.setAttribute('stroke', '#DAA520');
    ...
    function getRoutePointByDistance(distanceKm) {
        ...
        if (distanceKm <= LANGFANG_KM) {
            const t = clamp(distanceKm / LANGFANG_KM, 0, 1);
            return {
                x: lerp(routeGeo.dz.x, routeGeo.lf.x, t),
                y: lerp(routeGeo.dz.y, routeGeo.lf.y, t)
            };
        }
        const t = clamp((distanceKm - LANGFANG_KM) / (TOTAL_DISTANCE - LANGFANG_KM), 0, 1);
        return {
            x: lerp(routeGeo.lf.x, routeGeo.bj.x, t),
            y: lerp(routeGeo.lf.y, routeGeo.bj.y, t)
        };
    }
    ...
    function updateMiniMapProgress(distanceKm) {
        ...
        const t1 = clamp(distanceKm / LANGFANG_KM, 0, 1);
        const l1x = lerp(routeGeo.dz.x, routeGeo.lf.x, t1);
        const l1y = lerp(routeGeo.dz.y, routeGeo.lf.y, t1);
        miniMapData.doneLine1.setAttribute('x1', routeGeo.dz.x);
        miniMapData.doneLine1.setAttribute('y1', routeGeo.dz.y);
        miniMapData.doneLine1.setAttribute('x2', l1x);
        miniMapData.doneLine1.setAttribute('y2', l1y);

        if (distanceKm > LANGFANG_KM) {
            const t2 = clamp((distanceKm - LANGFANG_KM) / (TOTAL_DISTANCE - LANGFANG_KM), 0, 1);
            const l2x = lerp(routeGeo.lf.x, routeGeo.bj.x, t2);
            const l2y = lerp(routeGeo.lf.y, routeGeo.bj.y, t2);
            miniMapData.doneLine2.setAttribute('x1', routeGeo.lf.x);
            miniMapData.doneLine2.setAttribute('y1', routeGeo.lf.y);
            miniMapData.doneLine2.setAttribute('x2', l2x);
            miniMapData.doneLine2.setAttribute('y2', l2y);
        } else {
            miniMapData.doneLine2.setAttribute('x1', routeGeo.lf.x);
            miniMapData.doneLine2.setAttribute('y1', routeGeo.lf.y);
            miniMapData.doneLine2.setAttribute('x2', routeGeo.lf.x);
            miniMapData.doneLine2.setAttribute('y2', routeGeo.lf.y);
        }
    }
    ...
    function initStationScene() {
        ...
        camera.position.set(11.5, 4.5, 20);
        camera.lookAt(0, 2.0, -55);
        ...
        stationLightB.position.set(0, 18, -60);
        scene.add(stationLightB);

        stationGroup = new THREE.Group();
        scene.add(stationGroup);

        createStationDezhouEast();
        createFuxingTrain();
        createPersonModel();
        ...
        $('stationLabel').textContent = '西安北站';
        $('stationLabel').classList.add('show');
        showHint('西安北站发车前，先上 16 车，找到 10F！', 3600);

        playAudio('boarding');
        renderStation();

        setTimeout(() => {
            if (phase !== Phase.STATION_DZ) return;
            $('stationLabel').classList.remove('show');
            setTimeout(() => startBoarding(), 500);
        }, 4200);
    }

    function createStationDezhouEast() {
        ...
        for (const p of [
            [9.6, 0.6, -76], [8.6, 0.6, -8], [9.8, 0.6, 26], [8.8, 0.6, -50]
        ]) {
        ...
        const farStripeMat = new THREE.MeshLambertMaterial({ color: 0xC8A951 });
        for (let i = 0; i < 5; i++) {
    ...
    function createFuxingTrain() {
        ...
        const carHeight = 2.8;
        const gap = 0.3;
        const startCarNum = 9; // 16编组重联，显示第二单元（9-16车）
        for (let i = 0; i < carCount; i++) {
            const isHead = (i === 0);
            const isTail = (i === carCount - 1);
            const carNum = startCarNum + i;
            const car = createFuxingCar(isHead, isTail, carLength, carWidth, carHeight, carNum);
    ...
    function createFuxingCar(isHead, isTail, length, width, height, carNum) {
        const group = new THREE.Group();
        const bodyMat = new THREE.MeshLambertMaterial({ color: 0xf2f2f2 });
        const goldMat = new THREE.MeshLambertMaterial({ color: 0xC8A951 });
        ...
        if (isHead) {
            ...
            // 复兴号更流线型的车鼻
            const nose = new THREE.Mesh(new THREE.CylinderGeometry(0, width / 2 * 0.8, length * 0.48, 16), bodyMat);
            nose.rotation.x = Math.PI / 2;
            nose.position.z = length * 0.37;
            ...
            addFuxingLabel(group, width, height, length);
            addG88Label(group, width, height, length, true);
            ...
            if (carNum === 16) {
                addCarNumberLabel(group, width, height, '16车', true);
                addSeatMarkerLabel(group, width, '10F');
            ...
            const waist = new THREE.Mesh(new THREE.BoxGeometry(width + 0.04, 0.15, length + 0.04), goldMat);
            waist.position.y = -height * 0.3;
            group.add(waist);
    ...
    function addFuxingLabel(group, carWidth, carHeight, carLength) {
        ...
        ctx.strokeText('复兴号', 512, 138);
        ctx.fillStyle = '#C8A951';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('复兴号', 512, 138);
    ...
    function addG88Label(group, carWidth, carHeight, carLength, compact) {
        const c = document.createElement('canvas');
        c.width = 1024; c.height = 256;
        const ctx = c.getContext('2d');
        ctx.clearRect(0, 0, c.width, c.height);
    ...
    function addCarNumberLabel(group, carWidth, carHeight, numStr, isSpecial) {
        ...
        const ctx = c.getContext('2d');
        ctx.fillStyle = isSpecial ? '#B8860B' : '#444444';
    ...
    function createPersonModel(startX, startY, startZ) {
        ...
        personGroup.position.set(
            ...
            typeof startZ === 'number' ? startZ : 10
        );
    ...
    function startBoarding() {
        ...
        showHint('正在走向 16 车门...', 2600);
        updateStatusUI();

        // 16车在索引7（8车编组中的最后一车）
        const car16Z = 55 - 7 * (16 + 0.3);
        personTargetZ = car16Z + 5;
        personWalkTime = 0;
        personGroup.visible = true;
        personGroup.scale.set(1, 1, 1);
        personGroup.position.set(4, 0.36, 10);
    ...
    function animateBoardingWalk(timestamp) {
        ...
        const walkDuration = clamp((10 - personTargetZ) / 8.5, 5.5, 8.2);
        ...
        if (elapsed < walkDuration) {
            ...
            personGroup.position.z = lerp(10, personTargetZ, t);
            ...
            setTrainDoorOpenRatio(1);
            if (t > 0.3) showHint('上车完成，去 16 车 10F 座位！', 1200);
        } else {
            setPersonOpacity(1);
    ...
    function transitionToCabinSeating() {
        ...
        updateStatusUI();
        showHint('16车车厢内（一等座）：找到 10F 并坐下', 2200);
    ...
    function transitionToCabinAlight() {
        ...
        updateStatusUI();
        showHint('终点站到了，起身从 16 车门下车', 2200);
        animateCabinCutscene(cutsceneStart);
    }
    ...
    function initCabinScene(mode) {
        ...
        createPersonModel(
            mode === 'seating' ? 0.1 : cabinSceneData.seat.x,
            1.2,
    ...
    function createCabinInterior() {
        ...
        // 一等座 2+2 布局（比二等座宽敞）
        const panelMat = new THREE.MeshLambertMaterial({ color: 0x2F5496 });
        const baseMat = new THREE.MeshLambertMaterial({ color: 0x1F3864 });
        const rowZ = [5.8, 3.2, 0.6, -2.0, -4.6]; // 5排，间距更大
        rowZ.forEach((z, i) => {
            // 一等座2+2：左边两个(A,C)，右边两个(D,F)
            for (const x of [-1.55, -0.8, 0.8, 1.55]) {
                const seat = new THREE.Mesh(new THREE.BoxGeometry(0.58, 0.55, 0.82), panelMat);
                seat.position.set(x, 0.92, z);
                group.add(seat);
                const seatBase = new THREE.Mesh(new THREE.BoxGeometry(0.56, 0.7, 0.64), baseMat);
            ...
            if (i < rowZ.length - 1) {
                ...
                rowLine.position.set(0, 0.01, z - 1.28);
                group.add(rowLine);
            }
        });

        // 目标座位 10F（右侧靠窗）
        const targetSeat = { x: 1.55, z: -2.0 };
        ...
        sctx.font = 'bold 48px "Noto Sans SC", sans-serif';
        sctx.fillStyle = '#ffffff';
        sctx.textAlign = 'center';
        sctx.textBaseline = 'middle';
        sctx.fillText('16车  10F  一等座', 256, 64);
        ...
        return {
            ...
            door: new THREE.Vector3(0.1, 1.2, 8.5),
            aisleX: 0.1
        };
    }
    ...
    function animateCabinCutscene(timestamp) {
        ...
        if (phase === Phase.CABIN_SEATING) {
            if (elapsed < 2.4) {
                ...
                personGroup.rotation.y = Math.PI;
                setPersonSitPose(t);
                if (t > 0.5) showHint('10F 已入座，准备发车！', 1200);
            } else {
    ...
    function createDrivingTrain() {
        ...
        for (let i = 0; i < carCount; i++) {
            ...
            const isTail = (i === carCount - 1);
            const carNum = startCarNum + i;
            const car = createFuxingDrivingCar(isHead, isTail, carLength, carWidth, carHeight, carNum);
    ...
    function createFuxingDrivingCar(isHead, isTail, length, width, height, carNum) {
        const group = new THREE.Group();
        const bodyMat = new THREE.MeshLambertMaterial({ color: 0xf2f2f2 });
        const goldMat = new THREE.MeshLambertMaterial({ color: 0xC8A951 });
        const windowMat = new THREE.MeshLambertMaterial({ color: 0x5ba3e6, side: THREE.DoubleSide });
        ...
        if (isHead) {
            ...
            // 复兴号流线型车鼻
            const noseGeo = new THREE.CylinderGeometry(0, width / 2 * 0.8, length * 0.45, 16);
    ...
    // ============ HUD 更新 ============
    function updateHUD() {
        const pct1 = Math.min(100, (Math.min(journeyDistance, LANGFANG_KM) / LANGFANG_KM) * 100);
        $('progressFill').style.width = pct1 + '%';

        if (journeyDistance > LANGFANG_KM) {
            const pct2 = Math.min(100, ((journeyDistance - LANGFANG_KM) / (TOTAL_DISTANCE - LANGFANG_KM)) * 100);
            $('progressFill2').style.width = pct2 + '%';
        } else {
            $('progressFill2').style.width = '0%';
        }

        $('progressTrain').style.left = Math.min(95, (journeyDistance <= LANGFANG_KM ? pct1 : 100)) + '%';
    ...
    function animateDriving(timestamp) {
        ...
        const distToLangfang = LANGFANG_KM - journeyDistance;
        const distToEnd = TOTAL_DISTANCE - journeyDistance;
        const targetSpeedBase = CRUISE_SPEED * speedFactor;

        if (langfangState === 'approaching') {
            if (distToLangfang <= 0.5) {
                langfangState = 'stopping';
                journeyDistance = LANGFANG_KM;
                currentSpeed = 0;
                targetSpeed = 0;
            } else if (distToLangfang < LANGFANG_APPROACH_KM) {
                targetSpeed = Math.max(15, (distToLangfang / LANGFANG_APPROACH_KM) * targetSpeedBase);

                if (!speedMilestones['arriving-langfang'] && distToLangfang < LANGFANG_APPROACH_KM * 0.8) {
                    speedMilestones['arriving-langfang'] = true;
                    playAudio('arriving-langfang');
                }
            } else {
                targetSpeed = targetSpeedBase;
            }
        } else if (langfangState === 'stopping') {
            currentSpeed = 0;
            targetSpeed = 0;
            langfangState = 'stopped';
            langfangStopTimer = 0;
            $('stationLabel').textContent = '郑州东站';
            $('stationLabel').classList.add('show');
            playAudio('langfang-stop');
            showHint('郑州东站短暂停靠中...', 1500);
        } else if (langfangState === 'stopped') {
            currentSpeed = 0;
            langfangStopTimer += dt;
            if (langfangStopTimer > LANGFANG_STOP_DURATION) {
                langfangState = 'departing';
                $('stationLabel').classList.remove('show');
                playAudio('langfang-depart');
                showHint('郑州东发车，下一站终点北京西！', 1300);
            }
        } else if (langfangState === 'departing') {
            targetSpeed = targetSpeedBase;
            if (currentSpeed > targetSpeedBase * 0.9) {
                langfangState = 'passed';
            }
        } else if (langfangState === 'passed') {
            if (distToEnd < 15) {
                targetSpeed = Math.max(10, (distToEnd / 15) * targetSpeedBase);

                if (!speedMilestones['arriving-beijing']) {
                    speedMilestones['arriving-beijing'] = true;
                    playAudio('arriving-beijing');
                }
            } else {
                targetSpeed = targetSpeedBase;
            }
        ...
        if (!speedMilestones[200] && currentSpeed >= 200 && langfangState === 'approaching') {
            speedMilestones[200] = true;
            playAudio('speed-200');
        }
        if (!speedMilestones[300] && currentSpeed >= 295 && langfangState === 'approaching') {
            speedMilestones[300] = true;
            playAudio('speed-300');
        }
        if (!speedMilestones[350] && currentSpeed >= 345 && langfangState === 'approaching') {
            speedMilestones[350] = true;
            playAudio('speed-350');
        }
    ...
    function startArrivedPhase() {
        ...
        $('arrivalStats').textContent =
            '全程 ' + TOTAL_DISTANCE + ' 公里，途经郑州东，用时 ' + min + ' 分 ' + sec + ' 秒';
