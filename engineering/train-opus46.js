...
(function () {
    ...
    const AUDIO_FALLBACK = {
        ...
        'arriving-dezhou': '前方到站，郑州东。',
        'dezhou-stop': '郑州东站到了。',
        'dezhou-depart': '郑州东站开车，下一站，西安北。',
        'arriving-jinan': '前方终点站，西安北。',
        arrived: '西安北站到了！全程1200公里，感谢您乘坐G88次列车！',
    };
    ...
    const DEZHOU_APPROACH_KM = 20; // 距郑州东20km开始减速
    ...
    function phaseToStatusText() {
        if (phase === Phase.DRIVING) {
            ...
            if (dezhouState === 'stopped') return '停靠郑州东';
            if (dezhouState === 'stopping') return '进站减速';
            if (dezhouState === 'departing') return '郑州东发车';
            if (dezhouState === 'passed') return '驶向西安北';
            return '行驶中';
        }
    ...
    function addRouteOverlay(svgEl, bjEl, hnEl, sxEl, forMiniMap) {
        ...
        addCityMarker(svgEl, ns, bjCx, bjCy, '北京西', '', -12);
        addCityMarker(svgEl, ns, dzCx, dzCy, '郑州东', 'mid-label', -12);
        addCityMarker(svgEl, ns, jnCx, jnCy, '西安北', 'dest-label', 16);
    ...
    function initStationScene() {
        ...
        showHint('北京西站发车前，先上 06 车，找到 05D！', 3600);
    ...
    function createStationBeijingSouth() {
        ...
        lctx.fillText('北京西 → 郑州东 → 西安北', 512, 166);
